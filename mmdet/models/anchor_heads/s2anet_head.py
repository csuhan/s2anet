...
from mmdet.core import (AnchorGenerator, anchor_target_rotated, delta2rbox,
                        force_fp32, multi_apply, multiclass_ml_nms_rbox,
                        multiclass_nms_rbox, rect2rbox)
...
class S2ANetHead(nn.Module):
    ...
    def __init__(self,
                 ...
                 feat_channels=256,
                 stacked_convs=2,
                 align_conv_type='AlignConv',  # [AlignConv,DCN,GA_DCN,Conv]
                 align_conv_size=3,
                 with_orconv=True,
                 anchor_scales=[4],
                 anchor_ratios=[1.0],
    ...
    def forward_single(self, x, stride):
        ...
        if self.align_conv_type == 'AlignConv':
            ...
            align_feat = self.align_conv(x, align_offset)
        elif self.align_conv_type == 'GA_DCN':
            align_offset = self.align_conv_offset(fam_bbox_pred)
            align_feat = self.align_conv(x, align_offset)
        elif self.align_conv_type == 'Conv':
            align_feat = self.align_conv(x)
    ...
    def loss(self,
        ...
        # Feature Alignment Module tagets
